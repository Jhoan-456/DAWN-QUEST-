...
func sincronizar_arma_visual_rpc(ruta_textura: String) -> void:
	if ruta_textura != "":
		$PivotArma/ArmaVisual.texture = load(ruta_textura)
		$PivotArma/ArmaVisual.visible = true
	else:
		$PivotArma/ArmaVisual.visible = false
		
#====================================================================
@export_group("ATRIBUTOS DE FINNES")
@export var vida: float = 150.0
@export var max_vida: float = 150.0
@export var Escudo: float = 150.0
@export var max_escudo: float = 150.0
@export var Energia: float = 400.0
@export var max_energia: float = 400.0

@export var fuerza: int = 4
@export var inteligencia: int = 7
@export var Velocidad_ataque: float = 5.5	
@export var Velocidad_proyectil: float = 16.0
@export var Velocidad_movimiento: float = 10.2
@export var Probabilidad_crítico: float = 1.0 # 1%
@export var Suerte: float = 1.0
#=====================================================================

# =====================================================================
# 💧 PASIVA: MOJADO
# =====================================================================
@export var inmune_quemaduras: bool = true
@export var inmune_pozos_agua: bool = true
...
# 📖 ARMA INICIAL
# =====================================================================
@export_group("ARMA INICIAL")
@export var arma_inicial_scene: PackedScene 
...
var Moneda = 0
@onready var joystick = $UI/Joystick if has_node("UI/Joystick") else null
...
# 🟢 SISTEMA DE COOLDOWN PARA CAMBIAR ARMA
var puede_cambiar_arma: bool = true

var inventario_armas: Array[Dictionary] = []
var indice_arma_activa: int = 0

@onready var texto_moneda = $UI/LabelMoneda if has_node("UI/LabelMoneda") else null
@onready var texto_fps = $UI/fps if has_node("UI/fps") else null
@onready var texto_version = $UI/Version if has_node("UI/Version") else null
@onready var texto_segundos = $UI/segund_efect if has_node("UI/segund_efect") else null
...
func _ready() -> void:
	...
	if not is_multiplayer_authority():
		if has_node("UI"): $UI.visible = false
		if has_node("Camera2D"): $Camera2D.enabled = false
		return
	
	
	if has_node("Camera2D"): $Camera2D.enabled = true
	if texto_fps: texto_fps.visible = Datos.fps_visibles
	if texto_version: 
	...
	if arma_inicial_scene:
		var arma_instancia = arma_inicial_scene.instantiate()
		add_child(arma_instancia)
		equipar_arma(arma_instancia)
		arma_instancia.queue_free()
...
func _physics_process(delta: float) -> void:
	if is_multiplayer_authority():
		...
		if Input.is_action_pressed("mover_derecha"): direction.x += 1
		if Input.is_action_pressed("mover_izquierda"): direction.x -= 1
		if Input.is_action_pressed("mover_arriba"): direction.y -= 1
		if Input.is_action_pressed("mover_abajo"): direction.y += 1

		# 🟢 COMPROBAMOS SI EL COOLDOWN PERMITE CAMBIAR DE ARMA
		if (Input.is_action_just_pressed("cambiar_arma") or Input.is_action_just_pressed("c")) and puede_cambiar_arma:
			intercambiar_arma()
		...
		# 🟢 LÓGICA DEL PIVOTE PARA ROTAR EL ARMA ALREDEDOR DEL CUERPO
		if tiene_arma and has_node("PivotArma"):
			...
			if debe_rotar_arma:
				$PivotArma.rotation = angulo_apuntado
				var mirar_izquierda = abs(angulo_apuntado) > PI/2
				
				# Voltea el pivote para que el arma no quede de cabeza al apuntar a la izquierda
				if mirar_izquierda:
					$PivotArma.scale.y = -1
				else:
					$PivotArma.scale.y = 1
...
func _process(_delta: float) -> void:
	...
	if texto_fps and texto_fps.visible:
		var fps: int = Engine.get_frames_per_second()
		texto_fps.text = "FPS: " + str(fps)
		if fps <= 10: texto_fps.modulate = Color(1.0, 0.0, 0.0, 1.0)
		elif fps == 60: texto_fps.modulate = Color(0.0, 0.973, 0.339, 1.0)
		elif fps >= 100: texto_fps.modulate = Color(0.4, 0.898, 1.0, 1.0)
...
func equipar_arma(nodo_arma: Node2D) -> void:
	var datos_arma = {
		...
		"escena_suelo_path": nodo_arma.scene_file_path if "scene_file_path" in nodo_arma else "",
...
func intercambiar_arma() -> void:
	...
	# 🟢 BLOQUEAMOS EL CAMBIO DURANTE EL COOLDOWN
	puede_cambiar_arma = false 
	indice_arma_activa = 1 - indice_arma_activa
	activar_arma_actual()
	
	puede_cambiar_arma = true

func activar_arma_actual() -> void:
	if inventario_armas.is_empty():
		tiene_arma = false
		if has_node("PivotArma/ArmaVisual"): $PivotArma/ArmaVisual.visible = false
		if has_node("ArmaEspalda"): $ArmaEspalda.visible = false
		return

	tiene_arma = true
	if has_node("PivotArma/ArmaVisual"): $PivotArma/ArmaVisual.visible = true

	var arma = inventario_armas[indice_arma_activa]
	if "textura" in arma and arma["textura"] != null and has_node("PivotArma/ArmaVisual"):
		$PivotArma/ArmaVisual.texture = arma["textura"]
	...
	if arma["es_mago"]:
		...
		cadencia_calculada = max(0.05, arma["cad_base"] - (Velocidad_ataque * 0.03))
		...
		prob_critico_calculada = Probabilidad_crítico + 5.0
...
func disparar() -> void:
	if inventario_armas.is_empty(): return
	...
	if es_invocacion:
		...
		# 🟢 DISPARA DESDE EL NUEVO PIVOTE
		bala.global_position = $PivotArma/ArmaVisual/puntoDisparo.global_position
		bala.rotation = $PivotArma.rotation

	if "jugador" in bala: bala.jugador = self
	bala.set_multiplayer_authority(get_multiplayer_authority())

	if "direccion_vector" in bala:
		bala.direccion_vector = Vector2.RIGHT.rotated($PivotArma.rotation)
	
	var es_critico = (randf() * 100.0) <= prob_critico_calculada
	var dano_final = dano_calculado * (1.5 if es_critico else 1.0)
	
	if "daño" in bala: bala.daño = dano_final
	if "dano" in bala: bala.dano = dano_final
		
	if "velocidad" in bala: 
		bala.velocidad *= (arma_actual["mult_vel_p"])
...
func aplicar_retroceso():
	if not has_node("PivotArma/ArmaVisual"): return
	
	# 🟢 RETROCESO LIMPIO SOBRE EL PIVOTE
	var posicion_base = Vector2(15, 0) # Debe coincidir con la posición X que le pongas en el editor
	var posicion_disparo = Vector2(5, 0) # Se acerca al cuerpo al disparar
	
	$PivotArma/ArmaVisual.position = posicion_disparo
	var tween = create_tween()
	tween.tween_property($PivotArma/ArmaVisual, "position", posicion_base, 0.1)
...
func recibir_quemadura(cantidad: int, duracion: float):
	if inmune_quemaduras:
		crear_texto_flotante("¡Inmune!", Color(0.0, 0.8, 1.0))
		return

func entrar_pozo_agua():
	if inmune_pozos_agua:
		return
...
func recibir_veneno():
	...
	for i in range(tics_totales):
		...
		vida -= daño_por_tic
		vida = clamp(vida, 0, max_vida)
		if texto_segundos: texto_segundos.text = "Veneno: " + str(tiempo_espera)
...
func actualizar_orientacion_espaldas(mirar_izquierda: bool) -> void:
	...
	if has_node("ArmaEspalda"):
		...
		if mirar_izquierda:
			$ArmaEspalda.position.x = 4
			$ArmaEspalda.rotation_degrees = 45
		else:
			$ArmaEspalda.position.x = -4
			$ArmaEspalda.rotation_degrees = -45
